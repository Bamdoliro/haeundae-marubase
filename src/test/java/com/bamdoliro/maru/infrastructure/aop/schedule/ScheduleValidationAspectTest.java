...
class ScheduleValidationAspectTest {
    ...
    void 날짜_및_시간_검증에_성공한다() {
        try (MockedStatic<LocalDateTime> mockedLocalDateTime = mockStatic(LocalDateTime.class)) {
            mockedLocalDateTime.when(() -> LocalDateTime.now(Schedule.ZONE)).thenReturn(Schedule.START.plusSeconds(1));
    ...
    void 날짜_및_시간_검증에_실패하면_에러가_발생한다() {
        try (MockedStatic<LocalDateTime> mockedLocalDateTime = mockStatic(LocalDateTime.class)) {
            mockedLocalDateTime.when(() -> LocalDateTime.now(Schedule.ZONE)).thenReturn(Schedule.START.minusSeconds(1));
